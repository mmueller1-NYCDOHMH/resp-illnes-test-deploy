...
const DataPageLayout = ({
  ...
  pageBackground = "gray", contentGap = "40px",
        ...
        <div className="w-full max-w-[1280px] mx-auto flex flex-row items-stretch box-border">
          ...
          {sidebar && !isMobile && (
            <div className="relative flex-shrink-0 w-[220px] border-r border-[var(--gray-300)] bg-[var(--gray-100)] box-border">
              <div className="sticky top-[var(--header-h)] pt-4 px-2 pb-8">
                {sidebar}
              </div>
          ...
          <div className="flex-1 min-w-0 overflow-clip box-border flex flex-col">
            ...
            >
              ...
              {(!titleInContent || renderedSubtitle || topControls || info) && (
                <div
                  className={headerBackground ? "border border-black/[0.07] rounded-lg overflow-hidden" : "bg-white rounded-b-lg border border-[var(--gray-200)] shadow-sm overflow-hidden"}
                ...
                >
                  {/* Accent bar — blue by default, or the active virus's accent
                      ...
                      default, non-custom-background header). Card corners are
                      squared on top (rounded-b-lg above, not rounded-lg) so
                      this thicker bar reads as a clean flat top border instead
                      of curving into a rounded corner. */}
                  {!headerBackground && (
                    <div className="h-2" style={{ backgroundColor: "var(--page-accent, var(--blue-primary))" }} />
                  )}
                  <div className="p-4">
                    {!titleInContent && (

...
// ── Shared surface card ───────────────────────────────────────────────────────
// No horizontal padding here — the parent content wrapper already matches the
// title card's inset (DataPageLayout's px-4), so adding px-lg/px-md on top of
// that stacked an extra margin around every sub card that the title card
// didn't have.
const SurfaceCard = ({ id, children }) => (
  <div className="w-full max-w-content mx-auto mb-xl box-border">
    <section
      id={id}
...
const AboutPageLayout = ({ config }) => {
  ...
  return (
    ...
    >
      ...
      <div className="py-xl font-body md:py-lg">

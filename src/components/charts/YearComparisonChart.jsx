...
const YearComparisonChart = ({
  ...
  // subtype/group (colorField), falling back to the active virus. Only use
  // colorField when it actually varies across the data, to avoid a
  // constant-valued field duplicating the metric text.
  const colorFieldVaries =
    !!colorField &&
    new Set(filtered.map((d) => d[colorField]).filter((v) => v != null)).size > 1;
  const tooltipMetricLabel = columnLabels.value || metricName;
  const proportionTooltipLineCalc = buildTooltipLineCalc({
    seriesField: colorFieldVaries ? colorField : null,
    seriesLabel: virus || metricName,
    valueField: "pctDisplay",
    metricLabel: tooltipMetricLabel,
    isPercent: true,
  });
  const countsTooltipLineCalc = buildTooltipLineCalc({
    seriesField: colorFieldVaries ? colorField : null,
    seriesLabel: virus || metricName,
  ...
  const proportionTransform = [
    ...
    { calculate: "format(datum.pct, '.1%')", as: "pctDisplay" },
    { calculate: proportionTooltipLineCalc, as: "tooltipLine" },
  ];
  ...
  const countsTransform = [
    ...
    { calculate: "format(datum.value, ',d')", as: "valueDisplay" },
    { calculate: countsTooltipLineCalc, as: "tooltipLine" },
  ];

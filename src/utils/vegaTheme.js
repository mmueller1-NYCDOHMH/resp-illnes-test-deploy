...
/**
 * Shared per-chart boilerplate that individual chart files were
 * re-declaring near-identically (see SITE-AUDIT.md). Two different uses:
 *
 * 1. Spread into a spec's top-level `config.axis` / `axisX` / `axisY` /
 *    `view` / `legend` — VegaLiteWrapper always merges getVegaThemeConfig()
 *    on top of the spec afterwards, so the theme wins for every key these
 *    also set. Kept mainly for consistency/readability across files, not
 *    because the values are load-bearing there.
 * 2. Spread into a *per-encoding* axis (`encoding.x.axis` /
 *    `encoding.y.axis`) — NOT touched by the theme merge, so here the
 *    values are genuinely active (e.g. WastewaterChart's field-level axes).
 */
export const BASE_AXIS_LABEL_CONFIG = {
  labelFont: tokens.typography.body,
  titleFont: tokens.typography.heading,
  labelColor: tokens.colors.gray700,
  titleColor: tokens.colors.gray800,
  labelFontSize: 12,
};

export const BASE_AXIS_X_CONFIG = {
  ticks: true,
  domain: true,
...
export const BASE_AXIS_Y_CONFIG = {
  domain: false,
  ticks: false,
  tickCount: 3,
  orient: "left",
...
export const BASE_CHART_VIEW = { stroke: "transparent" };

export const BASE_LEGEND_CONFIG = {
  labelFont: tokens.typography.body,
  titleFont: tokens.typography.heading,
  labelColor: tokens.colors.gray600,
